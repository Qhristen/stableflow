...
use raydium_amm_cpi::{deposit, program::RaydiumAmm, Deposit};
...
pub struct Stake<'info> {
   ...
   pub token_program: Program<'info, Token>,
    ...
    impl<'info> Stake<'info> {
        pub fn deposit(&mut self, max_coin_amount:  u64, max_pc_amount: u64 ) -> Result<()> {
           ...
           deposit(ctx, max_coin_amount, max_pc_amount, 0)?;
